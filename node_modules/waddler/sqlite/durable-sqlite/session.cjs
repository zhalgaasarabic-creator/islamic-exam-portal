...
__export(session_exports, {
  DurableSqliteSQLTemplate: () => DurableSqliteSQLTemplate
});
module.exports = __toCommonJS(session_exports);
var import_errors = require("../../errors/index.cjs");
var import_sql_template = require("../../sql-template.cjs");
class DurableSqliteSQLTemplate extends import_sql_template.SQLTemplate {
  ...
  async *stream() {
    ...
    try {
      if (this.options.rowMode === "array") {
        const res = params.length > 0 ? this.client.sql.exec(query, ...params) : this.client.sql.exec(query);
        const stream2 = res.raw();
        for await (const row of stream2) {
          yield row;
        }
        return;
      }
      const stream = params.length > 0 ? this.client.sql.exec(query, ...params) : this.client.sql.exec(query);
