...
import { validateFactory as factoryValidator } from "./factoryValidator.js";
import { PooledResource } from "./PooledResource.js";
...
import { reflector } from "./utils.js";
class Pool {
  _config;
  promiseConstructor;
  _factory;
  ...
  _dispatchPooledResourceToNextWaitingClient(pooledResource) {
    ...
    if (clientResourceRequest === null || clientResourceRequest.state !== Deferred.PENDING) {
      this._addPooledResourceToAvailableObjects(pooledResource);
      return false;
    }
    const loan = new ResourceLoan(pooledResource, this.promiseConstructor);
  ...
  __allResourcesReturned() {
    const ps = [...this._resourceLoans.values()].map((loan) => loan.promise).map((element) => reflector(element));
    return Promise.all(ps);
  }
  // not in use for now
  async clear(ensureMinimum = true) {
    const reflectedCreatePromises = [...this._factoryCreateOperations].map((element) => reflector(element));
    await Promise.all(reflectedCreatePromises);
    for (const resource of this._availableObjects) {
      await this._destroy(resource, ensureMinimum);
    }
    const reflectedDestroyPromises = [...this._factoryDestroyOperations].map((element_1) => reflector(element_1));
