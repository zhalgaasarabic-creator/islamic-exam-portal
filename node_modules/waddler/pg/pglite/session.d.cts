import { PGlite } from '@electric-sql/pglite';
import { i as SQLTemplate, k as SQLWrapper, D as Dialect, h as SQLTemplateConfigOptions } from '../../index-GJ4oisb0.cjs';
import '@clickhouse/client/';
import '../../logger.cjs';
import 'pg-query-stream';

declare class PGliteSQLTemplate<T> extends SQLTemplate<T> {
    sqlWrapper: SQLWrapper;
    protected readonly client: PGlite;
    private options;
    private rawQueryConfig;
    private queryConfig;
    constructor(sqlWrapper: SQLWrapper, client: PGlite, dialect: Dialect, configOptions: SQLTemplateConfigOptions, options?: {
        rowMode: 'array' | 'object';
    });
    execute(): Promise<T[]>;
    /**
     * For now, throws the Error.
     * Current implementation (a placeholder) will be replaced once PGlite acquires streaming support or when a suitable third-party solution is found.
     */
    stream(): AsyncGenerator<never, void, unknown>;
}

export { PGliteSQLTemplate };
